...
- 应用版本在 `package.json`、`src-tauri/Cargo.toml` 与 `src-tauri/tauri.conf.json` 中保持一致，例如 `1.4.2`。
- 市场 MSIX 使用四段版本号，例如应用版本 `1.4.2` 对应清单版本 `1.4.2.0`。
- 每次应用代码变更完成后，更新 `src/App.vue` 的 `versionLabel`，格式固定为 `v1.4.2 - YYYY-MM-DD HH:mm`。
...
```text
outputs\Desktop-Sticky-Note_1.4.2.0_x64.msix
```
