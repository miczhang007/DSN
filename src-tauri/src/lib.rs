    ...
    tauri::Builder::default()
        .setup(|app| {
            ...
            if let Some(window) = app.get_webview_window("main") {
                let _ = place_window(&window, "top-right");
...
fn create_task(
    ...
    conn.execute(
        ...
        ",
        params![id, owner, trimmed_title, deadline_at, bool_to_i64(is_urgent), now],
    )
    .map_err(|err| err.to_string())?;
...
#[tauri::command]
fn rename_user_data(state: State<DbState>, old_owner: String, new_owner: String) -> Result<(), String> {
